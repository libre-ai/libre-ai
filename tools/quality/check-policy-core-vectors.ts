...
const rawInputCount = await verifyPolicyCoreRawInputVectors(failures, "policy-core-v1");
const schemaNames = [
  "common.v1.schema.json",
  "policy-definition.v1.schema.json",
  "model-snapshot.v1.schema.json",
  "policy-need.v1.schema.json",
...
const policyValidator = validator("policy-definition.v1.schema.json");
const snapshotValidator = validator("model-snapshot.v1.schema.json");
const needValidator = validator("policy-need.v1.schema.json");
const evaluationValidator = validator("policy-evaluation.v1.schema.json");

const golden = JSON.parse(
  await readFile("contracts/fixtures/policy-core-v1/golden.json", "utf8"),
) as { schemaVersion: string; engineVersion: string; cases: GoldenCase[] };
if (golden.schemaVersion !== "libre-ai.policy-core-golden-vectors.v1") {
...
for (const candidate of golden.cases) {
  ...
  const policyDigest = digest("libre-ai.policy-definition.v1", policySubject, "policy");
  if (
  ...
  if (
    ...
    digest("libre-ai.model-snapshot.v1", without(candidate.snapshot, "digest"), "snapshot")
  ) {
    failures.push(`${label}: invalid snapshot digest`);
  }
  if (
    candidate.need.digest !==
    digest("libre-ai.policy-need.v1", without(candidate.need, "digest"), "need")
  ) {
...
const operators = JSON.parse(
  await readFile("contracts/fixtures/policy-core-v1/operators.json", "utf8"),
) as {
...
if (operators.schemaVersion !== "libre-ai.policy-core-operator-vectors.v1") {
