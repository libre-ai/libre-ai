...
- compile le module import-free une fois et ne réutilise jamais une instance ou une mémoire linéaire.
...
Le harness `tools/qualification/notebook-core-v2/performance.playwright.ts` exécute le composant verrouillé dans un worker et une instance WASM neufs par opération. Pour chacun des trois moteurs épinglés et chacun des deux profils KDF ci-dessus, il effectue deux warm-ups puis 20 itérations mesurées de seal et open sur 16 MiB. Chaque open vérifie les 16 MiB octet par octet contre la fixture publique `0x5a`, liée au SHA-256 pré-calculé `55c7e25571a69216de25162f191bb2847201a09ee7efe46b5bada034acc695d5`, avant effacement best-effort. Cette vérification exacte évite une copie Web Crypto de qualification qui n'appartient pas au chemin produit ; les entrées et enveloppes restent transférées et détachées du producteur.

Le module WASM immuable et sans import est compilé une fois par page, après capture du RSS de référence, puis cloné structurellement dans chaque worker jetable. Seuls le code compilé est partagé : l'instance, la mémoire linéaire et tous les états cryptographiques restent neufs par opération. La durée réelle de cette compilation unique est conservée et ajoutée de façon conservatrice à **chaque** échantillon bout-en-bout. La fin de chaque worker appelle aussi `close()`, le host impose `terminate()`, puis un tour de boucle événementielle comptabilisé dans la mesure laisse traiter la destruction logique. Les campagnes trap/OOM utilisent toujours leurs artefacts et workers dédiés.
