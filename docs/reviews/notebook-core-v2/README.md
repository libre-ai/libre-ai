...
Statut : **locked / Gate A approuvée / cinq passes Gate B rejetées / release bloquée**.
...
Le propriétaire a consigné la décision `continue`; le moteur expérimental Notebook est autorisé après Gate A. La passe [`gate-b/5395e45`](gate-b/5395e45/) confirme le moteur Rust/WASM ; la passe [`gate-b/d0f643b`](gate-b/d0f643b/) ajoute un host de qualification vert sur Chromium, Firefox et WebKit ; la passe [`gate-b/7df396b`](gate-b/7df396b/) accepte sa remédiation de fermeture et d'effacement ; la passe [`gate-b/e9970b3`](gate-b/e9970b3/) qualifie la destruction du worker après trap, plafond mémoire ou blocage ABI ; la passe [`gate-b/593e99f`](gate-b/593e99f/) épingle et vérifie les archives runtime, injecte panic/OOM et refus serde/JCS/Argon, puis archive 20 itérations par profil et navigateur. Gate B reste rejetée : quatre budgets producteur échouent dans Firefox/WebKit, les classes contraintes et le host produit exact sont absents, et l'OOM processus/effacement physique ne sont pas prouvés. Aucune sauvegarde utilisateur ou release n'est autorisée.
...
L’artefact WASM et son composant transpillé ont des imports vides ; les copies ABI, buffers host et instances jetables après fautes injectées sont exercés dans trois navigateurs. Panic/OOM Rust, premier alloc serde, réservations JCS/Argon et matrice p50/p95/RSS sont désormais mesurés pour le harness sur une classe arm64 de référence. Le host produit, l'OOM du processus navigateur, l'effacement physique, les classes contraintes et les dépassements de budget restent à fermer.
