...
removes port 80 for HTTP and 443 for HTTPS, removes the fragment, removes dot segments per RFC 3986
section 5.2.4, emits `/` for an empty path, uppercases percent-escape hex digits and decodes
percent-encoded unreserved characters. Query order and all other query octets are preserved. The
...
`-0000`) are treated as zero. If UTC conversion would leave the year range 0001 through 9999, the
...
`item-invalid`. Rules must validate against `curation-rule-set.v2.schema.json`; rule IDs must be
unique; each non-date value must already equal `normalize-text(value, 500)`; a `sourceHost` value must
