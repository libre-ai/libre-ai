...
it introduces. A merge commit generated by the forge cannot carry an
author-matching trailer (its author is the forge account), so the maintainer
who merges must include their own `Signed-off-by:` line in the merge commit
