...
- the shared `engine-golden-vectors-v1` remains candidate: candidate-integration, Architecture and Security approve the combined IPv6/ASCII-metadata/wrapped-CFWS authority at `ccf9d68`; explicit owner acceptance of `entities@8.0.0` and a separate catalog-only promotion remain required ;
