...
- the shared `engine-golden-vectors-v1` remains candidate: successive integration/Security passes invalidated earlier approvals through RFC 6532 EAI, prose boundaries, generic URI userinfo, private-key markers, schema/scanner metadata drift and wrapped CFWS; exact candidate-integration now approves `bec32a7`, while fresh Architecture/Security and owner dependency acceptance remain required ;
