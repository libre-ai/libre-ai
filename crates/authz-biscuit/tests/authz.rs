use biscuit_auth::builder::{Algorithm, date, fact, string};
...
fn revocation_is_immediate_and_store_outage_denies() {
    ...
    .unwrap_err();
    assert_eq!(error.code, "auth.revocation_unavailable");
}
...
fn two_key_rotation_has_a_bounded_overlap() {
    ...
    assert_eq!(
        ...
        .unwrap_err()
        .code,
        "auth.key_rotation_invalid"
    );
    assert_eq!(
        ring.finish_rotation(now + Duration::from_secs(919))
            .unwrap_err()
            .code,
        "auth.key_rotation_not_ready"
    );
...
fn cache_ttl_and_input_bounds_are_enforced() {
    ...
    assert_eq!(
        BiscuitIssuer::new(39, p256, Duration::from_secs(60))
            .unwrap_err()
            .code,
        "auth.key_algorithm_invalid"
    );
    let p256 = KeyPair::new_with_algorithm(Algorithm::Secp256r1);
    assert_eq!(
    ...
    assert_eq!(
        RevocationChecker::new(MemoryRevocations::default(), Duration::from_secs(31))
            ...
            .unwrap()
            .code,
        "auth.revocation_cache_ttl_invalid"
    );
    assert_eq!(
    ...
    assert_eq!(
        revocations
            ...
            .unwrap_err()
            .code,
        "auth.revocation_record_invalid"
    );
