...
impl<S: RevocationStore> RevocationChecker<S> {
    pub fn new(store: S, cache_ttl: Duration) -> Result<Self, AuthzError> {
        if cache_ttl > Duration::from_secs(30) {
            return Err(AuthzError::new("auth.revocation_cache_ttl_invalid"));
        }
        Ok(Self {
            store,
            cache_ttl,
    ...
    pub fn check(&mut self, root_block_id: &str, now: SystemTime) -> Result<(), AuthzError> {
        ...
        let revoked = self
            ...
            .map_err(|_| AuthzError::for_root("auth.revocation_unavailable", root_block_id))?;
        self.remember(
            root_block_id,
            CachedStatus {
                revoked,
    ...
    pub fn revoke(&mut self, record: RevocationRecord, now: SystemTime) -> Result<(), AuthzError> {
        if record.root_block_id.len() != 64
            || !record
                .root_block_id
                .bytes()
        ...
        {
            return Err(AuthzError::new("auth.revocation_record_invalid"));
        }
